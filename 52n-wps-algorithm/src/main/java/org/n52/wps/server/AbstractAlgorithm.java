...
public abstract class AbstractAlgorithm implements IAlgorithm
{
	...
	public boolean processDescriptionIsValid() {
        LinkedList<XmlError> l = new LinkedList<>();
        boolean valid = getDescription().validate(new XmlOptions().setErrorListener(l));
        if (!valid && LOGGER.isWarnEnabled()) {
            LOGGER.warn("Generated invalid Process Description for {}:{}",
                        getWellKnownName(), Joiner.on("\n\t").join(l));
        }
		return valid;
	}
