...
public final class MSMapfileBinding {
	...
	private static final MSMapfileBinding instance = new MSMapfileBinding();

	private static final Logger LOGGER = LoggerFactory.getLogger(MSMapfileBinding.class);
	...
	public synchronized String addFeatureCollectionToMapfile(SimpleFeatureCollection ftColl, String workspacePath, String mapfilePath, String dataRepPath) {
        ...
        try {
            ...
            LOGGER.error("Error while saving FeatureCollection as a Shapefile to the data repository of the mapfile.", e);
        }
        ...
        try {
            ...
            LOGGER.error("Error while creating a temporary copy of the mapfile.", e);
        }
        ...
        try {
            ...
            LOGGER.error("Error while adding shapefile as a layer to the mapfile.", e);
        }
        ...
        try {
            ...
            LOGGER.error("Error while updating the original mapfile", e);
        }
	...
	private synchronized String addShapefileAsLayerToMapfile(
        ...
        try {
            ...
            try {
                ...
                LOGGER.error("Error while initializing MSLayerBinding object.", e);
            }
            ...
            LOGGER.error("Error while adding shapefile to mapfile.", e);
        }
     ...
     * @return
     */
	private String getMDSupportedCRS(
            String wmsSupportedCrs, String mdCrs) {
        String[] aCrs = wmsSupportedCrs.split(" ");
        for (String aCr : aCrs) {
            if (aCr.equalsIgnoreCase(mdCrs)) {
                return wmsSupportedCrs;
            }
        }
        return wmsSupportedCrs + " " + mdCrs;
    }

	/**
     * Creates a copy of the mapfile
     *
     * @param orgMapfilePath Path to the original mapfile.
     * @param tmpMapfilePath Path to the temporary mapfile.
     *
	...
	private synchronized boolean copyMapfile(String orgMapfilePath, String tmpMapfilePath) throws IOException {
        ...
        try {
            Files.copy(new File(orgMapfilePath), new File(tmpMapfilePath));
            LOGGER.debug("Creating of an temporary copy of the mapfile successful.");
            return true;
        } catch (IOException e) {
            LOGGER.error("Error while copying mapfile", e);
        }
     ...
     */
	private String saveFeatureCollectionToShapefile(SimpleFeatureCollection ftColl, String workspacePath, String dataRepository) throws IOException, URISyntaxException {
        ...
        SimpleFeatureType TYPE = ftColl.getSchema();
        ...
        */
        ...
        if (featureSource instanceof FeatureStore) {
            ...
            try {
                ...
                LOGGER.error("Error while saving FeatureCollection as shapefile.", e);
