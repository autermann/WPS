...
public class AsciiGrassParser extends AbstractParser {
    ...
    private static final Logger LOGGER = LoggerFactory
            .getLogger(AsciiGrassParser.class);

    public AsciiGrassParser() {
        super(AsciiGrassDataBinding.class);
    }

    @Override
    public AsciiGrassDataBinding parse(InputStream input, Format format) {
        GridCoverage2D grid = null;
        try {
            GridCoverageReader reader = new ArcGridReader(dumpToFile(input));
            grid = (GridCoverage2D) reader.read(null);
            LOGGER.info("getCoordinateReferenceSystem2D(): " + grid
                    .getCoordinateReferenceSystem2D().toString());
    ...
    private File dumpToFile(InputStream inputStream)
        ...
        File outputFile = File
                .createTempFile("temp" + inputStream.hashCode(), "tmp");
